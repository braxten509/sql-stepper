...
MYSQL_URL = (
    "https://cdn.mysql.com/Downloads/MySQL-8.0/mysql-8.0.46-linux-glibc2.17-x86_64-minimal.tar.xz"
)
...
RUNS_PER_MINUTE = int(
    os.environ.get("RUNS_PER_MINUTE", 600)
)  # per IP; high because a whole class can share one school IP
TRUST_PROXY = bool(
    os.environ.get("TRUST_PROXY")
)  # behind a proxy: the visitor is in X-Forwarded-For
MAX_STATEMENTS = 2000
...
AI_NOTE = os.environ.get(
    "AI_NOTE", "GLM 5.3 Flash on US servers that don't store or train on your chats."
)
...
def prepare_server():
    ...
    with connect() as c, c.cursor() as cur:
        ...
        )  # for a server started with a binary log
        cur.execute(
            r"SELECT schema_name FROM information_schema.schemata WHERE schema_name LIKE 'step\_%'"
        )
...
def start_mysql():
    try:
        with (
            connect() as c,
            c.cursor() as cur,
        ):  # already running (left over from a previous session)
        ...
        pass
    shutil.rmtree(
        HOME / "data", ignore_errors=True
    )  # the old case-sensitive folder (only scratch databases)
    install()
    proc = subprocess.Popen(
        [
            ...
            *MYSQLD,
            ...
            f"--datadir={DATA}",
            f"--socket={SOCK}",
            f"--pid-file={HOME / 'mysqld.pid'}",
            f"--log-error={HOME / 'mysqld.log'}",
        ]
    )
...
STMT_START = re.compile(
    r"\s*(select|with|insert|update|delete|create|drop|alter|truncate|replace)\b", re.I
)


PROGRAM = re.compile(
    r"\s*create\s+(?:definer\s*=\s*\S+\s+)?(function|procedure|trigger)\s+`?(\w+)`?", re.I
)
...
def split_sql(text):
    ...
    for chunk in split_semicolons(text):
        if PROGRAM.match(
            chunk
        ):  # a function's body has lines like "select ..." that are not new statements
...
def diff(before, after):
    ...
    for name, (cols, rows) in after.items():
        ...
        if (bcols, brows) == (cols, rows):
            ...
            olds = {
                i: {c: o for c, (o, n) in enumerate(zip(br, r)) if o != n}
                for i, (br, r) in enumerate(zip(brows, rows))
                if br != r
            }
            views.append(
                view(
                    ...
                    cols,
                    rows,
                    ["changed" if i in olds else None for i in range(len(rows))],
                    olds,
                    badge="changed",
                )
            )
            ...
            for r in brows:
                if keep[r] > 0:
                    ...
                    gone.append(r)
            views.append(
                view(name, cols, rows + gone, marks + ["removed"] * len(gone), badge="changed")
            )
...
MySQL.Generator.TRANSFORMS[exp.IntDiv] = lambda g, e: (
    f"{g.sql(e, 'this')} DIV {g.sql(e, 'expression')}"
)
...
def unwrap(e):
    ...
    while True:
        ...
        hidden = type(e).__name__.startswith("TsOrDs") or (
            len(kids) == 1 and not isinstance(e, LEAF) and e.sql("mysql") == kids[0].sql("mysql")
        )
...
def is_step(e):
    ...
    return (
        isinstance(e, (exp.Func, exp.Binary, exp.Unary, exp.In, exp.Between))
        and not isinstance(e, LEAF + (exp.Paren,))
        and (
            e.find(*LEAF) is not None
            or (isinstance(e, exp.Func) and not list(e.iter_expressions()))
        )
    )
...
def calc_steps(top, text, ref):
    ...
    def visit(e):
        e = unwrap(e)
        if (
            not is_step(e) or len(steps) >= 20
        ):  # ponytail: 20 steps per column; a longer formula shows its first 20
            return
        ...
        tpl = c.sql("mysql")
        if isinstance(e, exp.Func) and tpl.upper().startswith(
            e.sql_name() + "("
        ):  # IFNULL, not the COALESCE sqlglot writes
...
def inline_window(w, defs):
    ...
    def pick(k):
        return w.args.get(k) or base.args.get(k)

    return exp.Window(
        this=w.this.copy(),
        partition_by=[p.copy() for p in pick("partition_by") or []],
        order=pick("order").copy() if pick("order") else None,
        spec=pick("spec").copy() if pick("spec") else None,
    )
...
def inline_windows(node):
    ...
    for sel in list(node.find_all(exp.Select)):
        defs = {
            wd.this.name if isinstance(wd.this, exp.Expression) else str(wd.this): wd
            for wd in sel.args.get("windows") or []
        }
        if not defs:
            continue
        for w in list(sel.find_all(exp.Window)):
            full = (
                inline_window(w, defs)
                if w.args.get("alias") and w.find_ancestor(exp.Select) is sel
                else None
            )
            if full is not None:
...
def window_detail(w, ref, shown=None, text=""):
    ...
    over = (f"PARTITION BY {', '.join(part)} " if part else "") + (
        f"ORDER BY {', '.join(o.sql('mysql') for o in ords)}" if ords else ""
    )
    d = {
        "fn": fname(f, text),
        "sql": shown or w.sql("mysql"),
        "part": part,
        "order": [{"sql": o.this.sql("mysql"), "desc": bool(o.args.get("desc"))} for o in ords],
        "spec": w.args["spec"].sql("mysql") if w.args.get("spec") else None,
        "res": ref(w.sql("mysql")),
        "pos": ref(f"ROW_NUMBER() OVER ({over})"),
        "pvals": [ref(p) for p in part],
        "ovals": [ref(o.this.sql("mysql")) for o in ords],
    }
    if part:
    ...
    if (
        d["fn"] not in NO_FRAME
    ):  # the frame: COUNT(*) over it is its size; counted from the start it ends at the last row
        cnt = w.copy()
...
def checks(cond, text=""):
    ...
    for p in leaves:
        ...
        if isinstance(core, COMPARE + (exp.In, exp.Between)):
            ...
            cols.append(f"({left})")
            right = (
                core.expression.sql("mysql")
                if isinstance(core, COMPARE) and core.expression
                else None
            )
            if (
                right
                and full.startswith(left)
                and full.endswith(right)
                and not core.expression.find(exp.Query)
            ):
                ...
                if isinstance(core, exp.In) and core.args.get("query"):
                    d["query"] = (
                        core.args["query"].unnest().sql("mysql")
                    )  # Stepper.fill_lists runs it
            # YEAR(d) = 2020: also show how YEAR(d) came out
            sides = [core.this] + (
                [core.expression] if isinstance(core, COMPARE) and core.expression else []
            )
...
class Stepper:
    ...
    def statement(self, text, i):
        ...
        if prog:
            ...
            self.add(
                i,
                f"CREATE {kind} {name}",
                f"Made {kind.lower()} {name}. It runs when it gets called.",
                kind="change",
            )
            return
        try:
        ...
        if isinstance(node, exp.Query):
            ...
            for f in node.find_all(
                exp.Anonymous
            ):  # SELECT getNth(2): step inside the function first
                if f.name.lower() in self.funcs and all(
                    isinstance(a, exp.Literal) for a in f.expressions
                ):
    ...
    def change(self, text, node, i):
        ...
        tbl = (
            node.this
            if isinstance(node, (exp.Update, exp.Delete)) and isinstance(node.this, exp.Table)
            else node.find(exp.Table)
            if node
            else None
        )  # DELETE p1 FROM Person p1: name the real table
        t = (
            tbl.name.lower() if tbl else "the table"
        )  # lower_case_table_names=1 stores names lowercased
        if isinstance(node, exp.Insert):
            msg = f"INSERT added {plural(n, 'row')} to {t}. New rows are green."
        elif isinstance(node, exp.Update):
            msg = (
                f"UPDATE changed {plural(n, 'row')} in {t}. Changed cells are amber with the old value crossed out. "
            ...
            msg = (
                f"{t} already existed, so IF NOT EXISTS skipped this."
                if tbl and t in before
    ...
    def replay(self, f, args, auto=False):
        ...
        def bind(sql):  # a parameter or variable wins over a column with the same name, as in MySQL
            return sqlglot.parse_one(sql, read="mysql").transform(
                lambda x: (
                    sqlglot.parse_one(env[x.name.lower()], read="mysql")
                    if isinstance(x, exp.Column) and not x.table and x.name.lower() in env
                    else x
                )
            )
        ...
        self.add(
            i,
            f"Call {call}",
            "LeetCode calls your function with its own test values."
            if auto
            else f"The query calls {call}.",
            kind="note",
            scope=scope,
            sql=call,
        )
        ...
        for part in split_semicolons(f["body"]):
            d, st, r = (
                re.match(
                    r"declare\s+([\w\s,]+?)\s+\w+(?:\([^)]*\))?(?:\s+default\s+([\s\S]+))?$",
                    part,
                    re.I,
                ),
                re.match(r"set\s+`?(\w+)`?\s*=\s*([\s\S]+)$", part, re.I),
                re.match(r"return\b([\s\S]+)$", part, re.I),
            )
            if d:
                for x in d.group(1).split(","):
                    env[x.strip().lower()] = (
                        lit(value(bind(d.group(2)).sql("mysql"))) if d.group(2) else "NULL"
                    )
            elif st:
                    ...
                    "vars": [
                        {
                            ...
                            "after": cell(value(env[name.lower()])),
                        }
                    ],
                }
            elif r:
                ...
                if isinstance(node, exp.Subquery) and isinstance(node.unnest(), exp.Query):
                    self.text = r.group(
                        1
                    )  # function names in the RETURN query point into this text
                ...
                self.add(
                    i,
                    part[:60],
                    "This part of the function isn't stepped through; the result below is the real answer.",
                    kind="note",
                    scope=scope,
                )
                break
        if auto:  # a query that calls it shows the answer itself
            ...
            self.add(
                i,
                f"{call} returns",
                f"{call} returns {cell(rows[0][0]) if rows and rows[0][0] is not None else 'NULL'}. "
                f"To try another value, add a line like SELECT {f['name']}({', '.join('2' for _ in args)}); to your code.",
                kind="result",
                tables=[view(call, [call], rows)],
            )
    ...
    def find_rows(self, node):
        ...
        real = {
            s.alias_or_name.lower(): s.name.lower() for s in sources if isinstance(s, exp.Table)
        }  # names ignore case
        if isinstance(node, exp.Update):
            target = node.expressions[0].this.table or this.alias_or_name
        else:
            target = (
                node.args["tables"][0].alias_or_name
                if node.args.get("tables")
                else this.alias_or_name
            )
        table = real[target.lower()]
        cols, brows = self.snapshot()[table]
        tail = " ".join(
            node.args[k].sql("mysql") for k in ("where", "order", "limit") if node.args.get(k)
        )
        _, found = self.run(
            f"SELECT {'DISTINCT ' if len(sources) > 1 else ''}`{target}`.* FROM {this.sql('mysql')} {tail}"
        )
        used, positions = set(), []
        for r in found:
        ...
        if where and len(sources) == 1 and not node.args.get("limit"):
            ...
            if [
                r[: len(cols)] for r in rows2
            ] == brows:  # same row order as the snapshot, so they line up
                det.update(
                    type="filter",
                    rows=[[cell(x) for x in r[len(cols) :]] for r in rows2[:MAX_ROWS]],
                )
            else:
                det = None
    ...
    def row_steps(self, node, i, plan, after):
        ...
        head = (
            f"First MySQL finds the rows to {'update' if is_update else 'delete'}:{cond}. "
        ...
        self.add(
            i,
            where.sql("mysql") if where else f"Find rows to {'update' if is_update else 'delete'}",
            head + ("" if positions else " Nothing will change."),
            scope=scope,
            tables=[view(table, cols, brows, marks)],
            sql=where.sql("mysql") if where else None,
        )
        if det:
        ...
        for k, pos in enumerate(positions[:MAX_ROW_STEPS]):
            marks = [
                ("changed" if is_update else "removed") if m == "current" else m for m in marks
            ]
            marks[pos] = "current"
            if is_update:
                ...
                what = (
                    ", ".join(
                        f"{cols[c]}: {cell(o)} → {cell(arows[pos][c])}" for c, o in ch.items()
                    )
            ...
            extra = (
                f" ({plural(more, 'more row')} get the same treatment.)"
                if k == MAX_ROW_STEPS - 1 and more > 0
                else ""
            )
            self.add(
                i,
                f"Row {k + 1} of {len(positions)}",
                f"{what[0].upper() + what[1:]}.{extra}",
                kind="row",
                scope=scope,
                tables=[view(table, cols, rows, list(marks), dict(olds))],
            )
    ...
    def query(self, node, ctes, scope, stmt, raw=None):
        ...
        for cte in own.expressions if own else []:
            if own.args.get("recursive"):
                self.show(
                    with_sql(scope_ctes + [cte]) + f"SELECT * FROM `{cte.alias}`",
                    stmt,
                    f"CTE {cte.alias}",
                    f"WITH RECURSIVE {cte.alias}",
                    lambda n, p, a=cte.alias: (
                        f"The recursive CTE {a} starts with its first SELECT, then keeps "
        ...
        def done(n):
            return (
                f" This is the final result: {plural(n, 'row')}."
                if raw
                else f" This is the result of {scope}: {plural(n, 'row')}."
                if scope
                else ""
            )
        ...
        if isinstance(base, exp.SetOperation):
            ...
            why = {
                "UNION": "stacks both results on top of each other",
                "INTERSECT": "keeps only rows that appear in both results",
                "EXCEPT": "keeps rows from the left side that are not in the right side",
            }.get(op, "combines both results")
            self.show(
                final_sql,
                stmt,
                scope,
                f"{op}{'' if dedupe else ' ALL'}",
                lambda n, p: (
                    f"{op} {why}{', then removes duplicate rows' if dedupe else ' (ALL keeps duplicates)'}."
                    + done(n)
                ),
                final=True,
            )
        ...
        if not isinstance(base, exp.Select):
            self.show(
                ...
                scope,
                "Run query",
                lambda n, p: "Ran the query." + done(n),
                final=True,
            )
            return
        ...
        if (
            not frm
        ):  # SELECT (subquery) AS x: step through each subquery, then its answer becomes the value
            vals = []
            for e in base.expressions:
                ...
                vals.append(
                    {
                        "name": name,
                        "sql": e.unalias().sql("mysql"),
                        "sub": isinstance(e.unalias(), exp.Subquery),
                    }
                )
            self.show(
                final_sql,
                stmt,
                scope,
                "SELECT " + ", ".join(v["name"] for v in vals),
                lambda n, p: (
                    "No FROM, so SELECT just computes the values. A subquery used as a value gives its one "
                    "answer, or NULL when it finds no rows." + done(n)
                ),
                final=True,
                detail={"type": "values", "cols": vals},
            )
            return
        ...
        first = sources[0]
        what = (
            f"the result of derived table {first.alias}"
            if isinstance(first, exp.Subquery)
            else f"CTE {first.name}"
            if first.name in {c.alias for c in scope_ctes}
            else f"table {first.name}"
        )
        step(
            f"{pre}SELECT * FROM {first.sql('mysql')}",
            frm.sql("mysql"),
            lambda n, p: (
                f"FROM runs first: start with every row of {what} ({plural(n, 'row')})."
                + (
                    f" This is its own separate read of {first.name}: whatever this part does only shapes its own "
                    f"result, and the outer query's rows are not touched."
                    if scope and isinstance(first, exp.Table)
                    else ""
                )
            ),
        ...
        for k, j in enumerate(joins, 1):
            ...
            if not on:
                ...
                desc = (
                    f"pairs rows with matching rows of {tname}. As a {side} JOIN, rows from the "
            ...
            src = sources[k]
            if isinstance(src, exp.Table) and src.name.lower() in [
                x.name.lower() for x in sources[:k] if isinstance(x, exp.Table)
            ]:
                desc += f" ({src.name} is the same stored table read a second time, under the name {tname}.)"
            det, pcols = (
                checks(on, self.text) if isinstance(on, exp.Expression) else ({"checks": []}, [])
            )
            self.fill_lists(det, pre)
            step(
                f"{pre}SELECT {', '.join(f'`{a}`.*' for a in names[: k + 1])}{''.join(', ' + c for c in pcols)} {body}",
                j.sql("mysql").lstrip(", "),
                lambda n, p, d=desc, on=on: (
                    f"{'JOIN' if on else 'Comma join'} {d} {p} rows → {plural(n, 'row')}."
                ),
                labels=labels[: sum(len(c) for c in src_cols[: k + 1])],
                extra=len(pcols),
                detail={
                    "type": "join",
                    "left": sum(len(c) for c in src_cols[:k]),
                    "side": side,
                    "table": tname,
                    # the right table on its own, so the page can show both sides before they merge
                    "right": [
                        [cell(x) for x in r]
                        for r in self.run(f"{pre}SELECT * FROM {src.sql('mysql')}")[1][:MAX_ROWS]
                    ],
                    **det,
                },
            )
        ...
        if where:
            ...
            self.fill_lists(det, pre)
            step(
                f"{pre}SELECT {star}, ({where.this.sql('mysql')}) IS TRUE AS `__keep`, {', '.join(pcols)} {body}",
                where.sql("mysql"),
                lambda n, p: (
                    f"WHERE checks each row and keeps only rows where the condition is true "
                    f"(NULL counts as not true). Kept {plural(n, 'row')} of {p}; the crossed-out rows are dropped."
                ),
                labels=labels,
                keep=True,
                extra=len(pcols),
                detail={"type": "filter", **det},
            )
        ...
        def resolve(e):  # GROUP BY 1 / GROUP BY alias → the real expression
            ...
            return e.copy().transform(
                lambda x: (
                    alias_map[x.name].copy()
                    if isinstance(x, exp.Column)
                    and not x.table
                    and x.name in alias_map
                    and x.name not in all_cols
                    else x
                )
            )
        ...
        if group:
            ...
            found = top_aggs(
                base.expressions
                + ([resolve(having.this)] if having else [])
            ...
            step(
                f"{pre}SELECT {star}, DENSE_RANK() OVER (ORDER BY {ks}) AS `__grp`{''.join(', ' + c for c in argcols)} {body} ORDER BY {ks}",
                group.sql("mysql"),
                lambda n, p: (
                    f"GROUP BY gathers rows with the same {ks} into one group. Each color band below is one "
            ...
            other = [
                ...
                if not {norm(lab), norm(ref)} & {norm(k) for k in keys}
            ]
            try:
                cols, rows = self.run(
                    f"{pre}SELECT {', '.join(keys + [f'ANY_VALUE({r})' for _, r in other] + aggs)} "
                    f"{body} GROUP BY {ks} ORDER BY {ks}"
                )
                self.steps[-1]["detail"] = {
                    **(self.steps[-1].get("detail") or {}),
                    "aggs": ainfo,  # rows: what each aggregate reads per row
                    "type": "group",
                    "keys": keys,
                    "picked": len(other),
                    "summary": view(
                        "After GROUP BY", keys + [lab for lab, _ in other] + aggs, rows
                    ),
                    "aliases": {
                        e.this.sql("mysql"): e.alias
                        for e in base.expressions
        ...
        if having:
            ...
            self.fill_lists(det, pre)
            step(
                f"{pre}SELECT {cols + ', ' if cols else ''}({h.sql('mysql')}) IS TRUE AS `__keep`, {', '.join(pcols)} {body}"
                + (f" GROUP BY {', '.join(keys)}" if keys else ""),
                having.sql("mysql"),
                lambda n, p: (
                    f"HAVING filters whole groups (WHERE filters single rows, before grouping). "
                    f"Kept {plural(n, 'group')}; the crossed-out groups are dropped."
                ),
                keep=True,
                extra=len(pcols),
                detail={"type": "filter", "groups": True, **det},
            )
        ...
        if any(e.find(exp.Window) for e in base.expressions):
            notes.append(
                "Window functions (… OVER (…)) are calculated now, after WHERE, GROUP BY and HAVING, "
                "without merging rows."
            )
        if not group and any(
            a.find_ancestor(exp.Window, exp.Subquery) is None
            for e in base.expressions
            for a in e.find_all(exp.AggFunc)
        ):
            notes.append(
                "There is an aggregate but no GROUP BY, so all rows are treated as one group."
            )
        if any(e.find(exp.Case) for e in base.expressions):
            notes.append(
                "CASE is checked top to bottom for each row, and the first WHEN that is true wins."
            )
        cols_txt = ", ".join(
            e.alias if isinstance(e, exp.Alias) else e.sql("mysql") for e in base.expressions
        )
        n = step(
            final_sql if not later else pre + q.sql("mysql"),
            "SELECT " + cols_txt,
            lambda n, p: (
                f"SELECT now builds the output columns ({cols_txt}). "
                + " ".join(notes)
                + (done(n) if not later else "")
            ),
            final=not later,
        )
        if n is not None:
        ...
        if base.args.get("distinct"):
            ...
            step(
                final_sql if last else pre + q.sql("mysql"),
                "DISTINCT",
                lambda n, p: (
                    f"DISTINCT removes duplicate rows: {p} → {plural(n, 'row')}."
                    + (done(n) if last else "")
                ),
                final=last,
            )
        if base.args.get("order"):
            ...
            step(
                final_sql if last else pre + q.sql("mysql"),
                base.args["order"].sql("mysql"),
                lambda n, p: (
                    "ORDER BY sorts the rows (ascending unless DESC; in MySQL, NULLs sort first when "
                    "ascending)." + (done(n) if last else "")
                ),
                final=last,
            )
        if base.args.get("limit"):
            ...
            step(
                final_sql,
                (base.args["limit"].sql("mysql") + (" " + off.sql("mysql") if off else "")),
                lambda n, p: (
                    f"LIMIT keeps only {plural(n, 'row')}"
                    + (" after skipping the first ones" if off else "")
                    + f" out of {p}."
                    + done(n)
                ),
                final=True,
    ...
    def select_detail(self, base, q, pre):
        ...
        for e in base.expressions:
            ...
            d = {
                "name": e.alias_or_name or e.sql("mysql"),
                "sql": inner.sql("mysql"),
                "alias": isinstance(e, exp.Alias),
                "star": isinstance(inner, exp.Star),
                "window": bool(inner.find(exp.Window)),
                "agg": any(
                    not a.find_ancestor(exp.Window, exp.Subquery)
            ...
            if isinstance(inner, (exp.Case, exp.If)):
                ...
                d["case"] = {
                    "else": tail.sql("mysql") if tail else "NULL",
                    "if": isinstance(inner, exp.If),
                    "operand": inner.this.sql("mysql")
                    if isinstance(inner, exp.Case) and inner.this
                    else None,
                    "branches": [],
                }
                for iff in ifs:
                    cond = (
                        exp.EQ(this=inner.this.copy(), expression=iff.this.copy())
                        if d["case"]["operand"]
                        else iff.this
                    )
                    ...
                    d["case"]["branches"].append(
                        {
                            "when": iff.this.sql("mysql"),
                            "then": iff.args["true"].sql("mysql"),
                            "offset": len(extra),
                            **det,
                        }
                    )
                ...
                windows = [
                    w for w in inner.find_all(exp.Window) if not w.find_ancestor(exp.Subquery)
                ]
                if windows:
                    d["windows"] = [
                        window_detail(w, ref, w.meta.get("shown"), self.text) for w in windows
                    ]
        ...
        if found:  # no GROUP BY: all rows form one group
            args = []
            detail["aggs"] = [
                agg_info(a, lambda sql: args.append(sql) or len(args) - 1, self.text)
            ...
            try:
                for sqls, key in ((args, "agg_rows"), (list(found), "agg_values")):
                    if sqls:
                        ...
                        detail[key] = [
                            [cell(x) for x in r]
                            for r in self.run(pre + q3.sql("mysql"))[1][:MAX_ROWS]
                        ]
        ...
        if extra:  # run once more with the extra values, then line those rows up with the shown rows by value
            ...
            try:
                ...
                pool = [
                    (tuple(cell(x) for x in r[: -len(extra)]), [cell(x) for x in r[-len(extra) :]])
                    for r in rows
                ]
                detail["extra_rows"] = []
                for v in shown:
    ...
    def subqueries(self, exprs, owner, pre, ctes, scope, stmt):
        ...
        for s in [
            ...
            for s in e.find_all(exp.Subquery, exp.Exists)
        ]:
            q = s.this
    ...
    def show(
        ...
        try:
            ...
            self.add(
                stmt,
                title,
                f"Couldn't preview this stage (MySQL said: {e.args[-1]}).",
                kind="note",
                scope=scope,
            )
            return None
        ...
        name = (
            "Final result"
            if final and not scope
            else f"Result of {scope}"
            if final
            else "Rows at this stage"
        )
        self.add(
            stmt,
            title,
            text,
            kind="result" if final else "query",
            scope=scope,
...
def run_all(setup, code):
    ...
    def err(e):
        return (
            f"Stopped: your code ran longer than {RUN_SECONDS} seconds."
            if killed.is_set()
    ...
    try:
        ...
        st.add(
            None,
            "Starting tables",
            "Your schema ran. These are the tables before your code starts.",
            kind="start",
            tables=[view(n, c, r) for n, (c, r) in st.snapshot().items()],
        )
        for i, s in enumerate(stmts):
            if s["phase"] != "code":
                continue
            try:
                ...
                break
        called = " ".join(
            s["text"] for s in stmts if s["phase"] == "code" and not PROGRAM.match(s["text"])
        )
        for f in st.funcs.values():
            if not re.search(rf"\b{f['name']}\s*\(", called, re.I) and not any(
                x["kind"] == "error" for x in st.steps
            ):
                try:
...
CHAT_RULES = """You are a friendly SQL teaching assistant (TA) inside SQL Stepper, a website that steps \
through MySQL 8.0 code like a debugger: clause by clause in the order MySQL runs it (FROM, JOIN, WHERE, \
GROUP BY, HAVING, SELECT, DISTINCT, ORDER BY, LIMIT), showing the table at each step. Students use it to \
learn SQL and practice LeetCode-style problems. You get the problem they're solving (if they pasted it), \
their schema, their code, and the step they are looking at.
...
often the best hint. A small example on different data is fine.
- If they're still stuck after a couple of hints, offer to show the answer and wait for a yes.
- Fully explaining a step, an error message, or a SQL concept is fine; that isn't giving the answer.
- Be correct above all, and say so when you're not sure. An empty result is not the same as one row \
holding NULL (LeetCode checks the difference).
- Be concise: usually 2 to 5 sentences (under about 100 words), one main idea per reply. No long lists, \
no repeating what they can already see, no filler. Go longer only when they ask for more or for the solution.
- Don't guess which problem they're solving if it isn't given; ask if it matters.
- Be plain and encouraging. Use their actual table and column names and values. Put SQL in ```sql blocks.
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if size > MAX_BODY:
            return self.reply(
                {"error": "That's too much for one request (the limit is about 200 KB)."}
            )
        who = (
            self.headers.get("X-Forwarded-For", "").split(",")[0].strip() if TRUST_PROXY else ""
        ) or self.client_address[0]
        if self.path == "/chat":
            return self.chat(json.loads(self.rfile.read(size)))
        if too_many(who):
            return self.reply(
                {"error": "Too many runs in the last minute. Wait a bit and try again."}
            )
    ...
    def chat(self, data):
        ...
        if not AI_KEY:
            return self.send(
                503, b"The AI chat isn't set up on this server.", "text/plain; charset=utf-8"
            )
        if PROXY_SECRET and not secrets.compare_digest(
            self.headers.get("X-Proxy-Secret", ""), PROXY_SECRET
        ):
            return self.send(
                403, b"Use the chat at sqlstepper.psbhr.com.", "text/plain; charset=utf-8"
            )
            ...
            json.dumps(
                {
                    ...
                    "provider": {"only": AI_HOSTS, "zdr": True, "data_collection": "deny"},
                }
            ).encode(),
            {
                "Authorization": f"Bearer {AI_KEY}",
                "Content-Type": "application/json",
                "HTTP-Referer": "https://sqlstepper.psbhr.com",
                "X-Title": "SQL Stepper",
            },
        )
        try:
            ...
            if e.code in (402, 403) and re.search(rb"limit|credit", why, re.I):  # the daily $ cap
                ...
                return self.send(
                    502,
                    b"The AI helper has reached today's limit. It will be back tomorrow.",
            ...
            msg = (
                ...
                if e.code == 429
                else "The AI couldn't answer right now. Try again in a bit."
            )
